    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          Eigen::Map<Eigen::VectorXd> vptsx(ptsx.data(), ptsx.size());
          Eigen::Map<Eigen::VectorXd> vptsy(ptsy.data(), ptsy.size());

          Eigen::VectorXd coeffs = polyfit(vptsx, vptsy, 3);
          
          double delay = 0.1; // 0.1s

          double x_delay = v * delay;
          double y_delay = 0;
          double psi_delay = delay * steering_angle/2.67;
          ...
          MpcResult result = mpc.Solve(state, coeffs);

          double steer_value = result.steering_angle/deg2rad(25);
          double throttle_value = result.throttle;
